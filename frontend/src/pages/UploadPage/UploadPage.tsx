...
export default function UploadPage() {
  ...
  const uploadHandler = async () => {
    ...
    try {
      ...
      const initRes = await axios.post<{ uploadId: string }>(
        "http://localhost:3000/upload/init",
        initFormData,
        {
          headers: {
      ...
      for (let i = 0; i < totalChunkSize; i++) {
        ...
        const uploadPromise = axios.post<string>(
      ...
      // finalize the upload
      const completeRes = await axios.post<string>(
        "http://localhost:3000/upload/complete",
        {
          fileName: video.name,
    ...
    <div className="bg-red-100 w-full h-full ">
      ...
      >
        ...
        <button
          ...
          disabled={!video || title === "" || username === ""}
          className="disabled:bg-gray-200 bg-black disabled:text-black text-white"
        >
          Upload
        </button>
      </form>
